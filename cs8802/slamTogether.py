...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    dim = N + num_landmarks
    Omega = matrix([[0 for i in range(dim)] for j in range(dim)])
    XiX = matrix([[0] for j in range(dim)])
    XiY = matrix([[0] for j in range(dim)])

    # set up initial position
    Omega += matrix([[1.]]).expand(dim, dim, [0], [0])
    XiX += matrix([[50.]]).expand(dim, dim, [0], [0])
    XiY += matrix([[50.]]).expand(dim, dim, [0], [0])

    n1 = 1. # /motion_noise
    n2 = 1. # /measurement_noise
    for i in range(len(data)):
        d = data[i]
        (xMovement, yMovement) = d[1]
        movementExpand = [i, i+1]
        Omega += matrix([[n1, -n1], [-n1, n1]]).expand(dim, dim, movementExpand, movementExpand)
        XiX += matrix([[-xMovement], [xMovement]]).expand(dim, 1, movementExpand, [0])
        XiY += matrix([[-yMovement], [yMovement]]).expand(dim, 1, movementExpand, [0])

        for (landmarkIdx, xLandmark, yLandmark) in d[0]:
            landmarkExpand = [i, N + landmarkIdx]
            Omega += matrix([[n2, -n2], [-n2, n2]]).expand(dim, dim, landmarkExpand, landmarkExpand)
            XiX += matrix([[-xLandmark], [xLandmark]]).expand(dim, 1, landmarkExpand, [0])
            XiY += matrix([[-yLandmark], [yLandmark]]).expand(dim, 1, landmarkExpand, [0])

    omegaInverse = Omega.inverse()
    muX = omegaInverse * XiX
    muY = omegaInverse * XiY
    mu = matrix([[0] for i in range(2 * dim)])
    for i in range(dim):
        mu.value[i * 2] = muX.value[i]
        mu.value[i * 2 + 1] = muY.value[i]
