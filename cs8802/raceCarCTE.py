...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        #            
        if self.x < radius: # left semi-circle
            cte = self.dist([radius, radius]) - radius

        elif self.x > 3 * radius: # right semi-circle
            cte = self.dist([3 * radius, radius]) - radius

        elif self.orientation > -pi / 2.0 and self.orientation < pi/2.0: # top
            cte = self.y - 2 * radius
