...
import copy
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00000000001):
    ...
    while maxDelta > tolerance:
        ...
        for i in range(len(path)):
            ...
            for j in range(len(path[0])):
                ...
                newpath[i][j] += weight_smooth * (newpath[iPrev][j] + newpath[iNext][j] - 2 * newpath[i][j])
                maxDelta = max(maxDelta, abs(oldValue - newpath[i][j]))
        """
        datapath = copy.deepcopy(newpath)
        for i in range(len(path)):
            for j in range(len(path[0])):
                datapath[i][j] += weight_data * (path[i][j] - newpath[i][j])

        smoothpath = copy.deepcopy(datapath)
        for i in range(len(path)):
            iPrev = (i - 1) % len(path)
            iNext = (i + 1) % len(path)
            for j in range(len(path[0])):
                smoothpath[i][j] += weight_smooth * (datapath[iPrev][j] + datapath[iNext][j] - 2 * datapath[i][j])
                maxDelta = max(maxDelta, abs(smoothpath[i][j] - newpath[i][j]))
...
#
def solution_check(newpath, answer, path=None, weight_data = 0.1, weight_smooth = 0.1):
    ...
    if path:
        ...
        for i in range(len(newpath)): 
            for j in range(len(newpath[0])):
                iPrev = (i - 1) % len(newpath)
                iNext = (i + 1) % len(newpath)
                newpathError += weight_data * (path[i][j] - newpath[i][j]) ** 2
                # newpathError += weight_smooth * (newpath[iPrev][j] - newpath[i][j]) ** 2
                newpathError += weight_smooth * (newpath[iNext][j] - newpath[i][j]) ** 2

                answerError += weight_data * (path[i][j] - answer[i][j]) ** 2
                # answerError += weight_smooth * (answer[iPrev][j] - answer[i][j]) ** 2
                answerError += weight_smooth * (answer[iNext][j] - answer[i][j]) ** 2
        print "New Path Error:{0}, Answer Error:{1}".format(newpathError, answerError)
...
# solution_check(smooth(testpath1), answer1, testpath1)
