...
def plotPath(path):
    if subplot:
        subplot.plot([b for a,b in path], [a for a,b in path])
    ...
    if subplot:
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j]:
                    # offset collisions to be centered at integer values
                    subplot.add_patch(mpl.patches.Rectangle((j-.5, i-.5), 1, 1, color='red'))
        plt.xlim([0-.5, len(grid[0])-.5])
        plt.ylim([0-.5, len(grid)-.5])

def plotRobot(robot, estimate, cte, color):
    global markerScale
    if subplot:
        subplot.plot(robot.y, robot.x, color=color, marker='+', alpha=.3)
        subplot.plot(estimate[1], estimate[0], color=color, marker='o', markersize=20 * cte + 1, alpha=.3)
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000, graph = False):

    #win, resolution = winAndResolution
    #pt = Point(len(grid)/2*resolution, len(grid[0])/2*resolution)
    #circle = Circle(len(grid)/3*resolution, len(grid[0])/3*resolution)
    #pt.draw(win)
    #circle.draw(win)
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        def calcProjectionAndDist(spath, index):
            # calculate our vectors
            startSegment = spath[index]
            endSegment = spath[index + 1]
            segmentVec = [endSegment[0] - startSegment[0], endSegment[1] - startSegment[1]]

            # calculate projection of location onto segment
            segmentLength = sqrt(segmentVec[0] ** 2 + segmentVec[1] ** 2)
            locVec = [estimate[0] - startSegment[0], estimate[1] - startSegment[1]]
            projection = (segmentVec[0] * locVec[0] + segmentVec[1] * locVec[1]) / segmentLength
            distToSegment = (-segmentVec[1] * locVec[0] + segmentVec[0] * locVec[1]) / segmentLength

            return (projection, distToSegment)

        (projection, distToSegment) = calcProjectionAndDist(spath, index)
        if projection > 1:
            index = min(index+1, len(spath)-1)
            (projection, distToSegment) = calcProjectionAndDist(spath, index)

        cte = distToSegment 

        global colors, runCount
        plotRobot(myrobot, estimate, cte, colors[runCount % len(colors)])
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
     weight_data, weight_smooth, p_gain, d_gain, graph = False):
    ...
    return run(grid, goal, path.spath, [p_gain, d_gain], graph = graph)
...
graph = True
colors =['red', 'blue', 'green', 'pink', 'purple', 'black', 'gray', 'orange', 'yellow']

for runCount in range(20):
    result = main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
           weight_data, weight_smooth, p_gain, d_gain, graph)
    if result[0]:
        successes += 1
    collisions += result[1]
    totalSteps += result[2]

print "weight_data:{0}, weight_smooth:{1}, p_gain:{2}, d_gain:{3}".format(weight_data, weight_smooth, p_gain, d_gain)
print "Success Rate:{0}, Collision Rate:{1}, StepRate:{2}".format(successes/20., collisions/20., totalSteps/20.)

if subplot and graph:
    plt.show()
