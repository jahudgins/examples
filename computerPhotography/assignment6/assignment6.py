...
def gaussPyramid(image, levels):
  ...
  while levels > 0 and min(image.shape) > 1:
      image = reduce(image)
      output.append(image)
      levels -= 1
...
def laplPyramid(gaussPyr):
  ...
  Returns:
    output (list): A laplacian pyramid of the same size as gaussPyr. This
                   pyramid should be represented in the same way as gaussPyr, 
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
  ...
  for laplWhite, laplBlack, gaussMask in zip(laplPyrWhite, laplPyrBlack, gaussPyrMask):
      blended_pyr.append(np.multiply(gaussMask, laplWhite) + np.multiply(1-gaussMask, laplBlack))
...
def collapse(pyramid):
  ...
  for image in pyramid[1:]:
    ...
    cropped = expanded[0:image.shape[0], 0:image.shape[1], ...]
